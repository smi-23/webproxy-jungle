...
// {
//     int listenfd, connfd;                  // listen socket과 connection socket 선언
//     char hostname[MAXLINE], port[MAXLINE]; // connection 을 위한 hostname과 port 선언
...
//     if (argc != 2)
//     {
//         // 인자를 두개 미만이나 초과로 입력하면 에러메시지를 출력
...
//         // Getnameinfo는 위를 반대로 소켓 주소 구조체에서 스트링(호스트 주소, 포트 번호) 표시로 변환
...
//     { // 11.11 숙제 문제 = method 가 HEAD일 때 body를 보내지 않기 위함
...
//         }
//         // 자식 프로세스가 실행되고 결과를 출력하고 종료될 때까지 기다림
